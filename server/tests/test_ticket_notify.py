...
async def test_notify_only_on_done():
    db = AsyncMock()
    db.fetchrow.return_value = {"id": 1, "status": "qa", "title": "t", "parent_id": None}
...
async def test_no_notify_on_non_done():
    ...
    for frm, to in (("todo", "progressing"), ("progressing", "qa"), ("progressing", "cancel")):
        db = AsyncMock()
        db.fetchrow.return_value = {"id": 1, "status": frm, "title": "t", "parent_id": None}
        db.fetchval.return_value = 1
...
async def test_epic_auto_done_when_all_children_closed():
    ...
    db.fetchrow.side_effect = [
        {"id": 2, "status": "qa", "title": "child", "parent_id": 10},  # 전이 대상(하위)
        {"id": 10, "status": "progressing", "title": "epic"},          # 부모 에픽
    ]
...
async def test_epic_not_done_when_child_still_open():
    db = AsyncMock()
    db.fetchrow.side_effect = [
        {"id": 2, "status": "qa", "title": "child", "parent_id": 10},
        {"id": 10, "status": "progressing", "title": "epic"},
    ]
